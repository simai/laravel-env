...
- `site add`:
  - Profiles: `laravel` (requires `artisan`, root `/home/.../public`), `generic` (root `/home/.../public`, placeholder page).
  ...
  - Copies `templates/healthcheck.php` to `public/healthcheck.php`.
- `site remove`: choose domain from list if missing; yes/no prompts for removing files/DB/user; removes nginx/site configs, php-fpm pools, optional files/db/user; stays in menu on errors.
- `site list`: lists nginx sites-available.
...
## Templates
- `templates/nginx-laravel.conf`: root `{{PROJECT_ROOT}}/public`, PHP-FPM socket per project.
- `templates/nginx-generic.conf`: root `{{PROJECT_ROOT}}/public`.
